...
class TestSync(AuchAppTest):
    ...
    def get_manifest_resource(self, modified=None, **kwargs):
        kwargs = {}
    ...
    def test_sync_data_with_modifiedsince_header_missing(self):
        response = self.get_manifest_resource()
        self.assertBadRequest(response)

    def test_sync_data_with_empty_modifiedsince_header(self):
        response = self.get_manifest_resource(modified=' ')
        self.assertBadRequest(response)
...
class TestSyncProduct(TestSync):

    def get_product_resource(self, pfile, **kwargs):
        kwargs = {}
        headers = {'Authentication-Token' : self.token}
        kwargs['headers'] = headers
        
        return self.test_app.open('/api/sync/%s' % pfile, method='get', **kwargs)

    def test_sync_product_with_invalid_filename_no_suffix(self):
        response = self.get_product_resource('s1_v1')
        self.assertNotFound(response)

    def test_sync_product_with_invalid_filename_no_version(self):
        response = self.get_product_resource('s1.json')
        self.assertNotFound(response)

    def test_sync_product_with_invalid_filename_invalid_version(self):
        response = self.get_product_resource('s1_1.json')
        self.assertNotFound(response)

    def test_sync_product_with_invalid_filename_no_target(self):
        response = self.get_product_resource('_v1.json')
        self.assertNotFound(response)

    def test_sync_product_with_invalid_filename(self):
        response = self.get_product_resource('thisisinvalid')
        self.assertNotFound(response)

    def test_sync_product_with_valid_filename_but_target_doesnot_exist_for_user(self):
        response = self.get_product_resource('s6_v1.json')
        self.assertNotFound(response)

    def test_sync_product_with_valid_filename_but_version_invalid(self):
        response = self.get_product_resource('s1_v99.json')
        self.assertNotFound(response)

    def test_sync_product_with_valid_filename(self):
        response = self.get_product_resource('s1_v1.json')
        self.assertOk(response)

    def test_sync_product_with_valid_filename_but_no_token(self):
        self.token = ''
        response = self.get_product_resource('s1_v1.json')
        self.assertNotAuthorized(response)
